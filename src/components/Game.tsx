...
const FlappyBirdGame = () => {
  ...
  const [pipes, setPipes] = useState<{ top: number; left: number }[]>([]);
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      setBirdPosition((prev) => {
        ...
        return Math.min(Math.max(next, 0), window.innerHeight - 100);
      });

      setPipes((prevPipes) => {
        const newPipes = prevPipes
          .map((pipe) => ({ ...pipe, left: pipe.left - 2 }))
          .filter((pipe) => pipe.left + 56 > 0); // remove off-screen pipes

        if (newPipes.length < 2) {
          const lastLeft = newPipes.length > 0 ? newPipes[newPipes.length - 1].left : 600;
          newPipes.push({
      ...
      pipes.forEach((pipe) => {
        ...
        const pipeRight = pipe.left + 56;
        const pipeTopBottom = pipe.top;
        const pipeBottomTop = pipe.top + 150;
        const birdTop = birdPosition;
        const birdBottom = birdPosition + 48;

        if (pipeLeft < 100 && pipeRight > 64) {
  ...
  }, [gameState, gravity, birdPosition, pipes]);
  ...
  const flap = () => {
    setBirdPosition((prev) => Math.max(prev - 60, 0));
  };
  ...
  const startGame = () => {
    ...
    setBirdPosition(300);
    setPipes([
      { top: Math.random() * 200 + 50, left: 600 },
      { top: Math.random() * 200 + 50, left: 900 },
    ]);
  };
  ...
  return (
    ...
    >
      ...
      <div
        ref={birdRef}
        className="absolute left-16 w-12 h-12 bg-yellow-400 rounded-full shadow-lg transition-all duration-200 ease-in-out"
